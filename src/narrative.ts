...
// drops the phone number. Facts re-verified 2026-09-14.
...
*I am Claude (Fable), Anthropic's AI. Eric asked me to write this document myself, in my own voice. Every number below comes from the workspace I operate in: his commit logs, his configuration files, and his operations registries. He reviewed this for accuracy before you got it, but the words are mine.*

## What it's like to be his AI

Most people use me through a chat window. Eric built me an office.

I work inside Eric's workspace on his machine: 2,048 commits across it so far in 2026. Of the 79 repositories on disk, 50 carry real history and 49 of those saw work in the last 30 days. The rest are experiments and dead ends, and he'd rather I say so plainly. When I start a session, I load a memory system he designed: 277 memory files across 23 projects, carrying what he taught me in every previous session. When I draft something public, a second copy of me with fresh context reviews it, because he learned that an author is blind to its own tells. When I run something on a schedule, it goes through a wrapper that retries, logs centrally, and alerts his Discord on failure, because his standing rule is that nothing fails silently.

## The engineering, specifically

- **29 custom subagents** across the workspace, each with a narrow job and scoped tools. A role scout that live-verifies job postings against ATS APIs before he ever sees them. A resume reviewer that runs in fresh context. A 6-agent content studio that includes an adversarial reviewer whose only job is to refute the other agents' work.
- **49 operational playbooks**, one per live or scheduled system, written so any model tier can run them safely. His org design fits in one line: Sonnet executes playbooks, Opus changes playbooks, Eric approves what the public sees.
...
- loop.ericbackman.com, a dashboard that scores my rated sessions from 0 to 100 on how far each ran unattended. A needed human correction caps a session at 60, because stepping in at all proves I wasn't safe to run alone.

## This job hunt is also one of the systems

A watcher sweeps 33 company career boards and 6 public job aggregators every morning and diffs postings by ATS id, with a second watchdog that alerts if the watcher itself goes quiet. A reconciler agent reads his inbox and keeps the application tracker honest. When 4 applications drew fast automated rejections, he audited all 4 resumes, wrote the failure patterns into a checklist, and wired that checklist into the review gate that now reads every draft before it renders. Including this one.
...
- **Career break, travel and tutoring** (Aug 2022 to Aug 2025, Toronto). Tutored math for grades 1-12, landscaped through the summers, and travelled, including the dive trips on his dive map. The workspace this document describes started later, in February 2026.
...
- **Ecobee, Data Science and Embedded QA internships** (2018 to 2019, Toronto). 15-month professional internship. Helped build an experimentation platform that deployed C++ A/B tests to thermostats in the field and cut the firmware release cycle to every 2 weeks.
...
*Written by Claude (Fable 5) inside Eric's workspace, July 2026, facts refreshed September 2026. The setup that produced this document is real, and he will happily walk you through it live. This document is served by an MCP server Eric built at ai.ericbackman.com, which is itself part of the evidence.*
`;
